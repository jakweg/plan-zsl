...
export class TimetableDatabase {
    ...
    private static mapByXmlEntities(xml: Element, entityName: string, constructor: Function, groupKey: string = 'id'): any {
        const parsed = Array.from(xml.querySelectorAll(entityName)).map((element) => constructor(element))
        if (!groupKey)
            return parsed;
        const tmp = {};
                ...
                for (const card of cards) {
                    ...
                    for (const group of lesson.groupIds
                        ...
                        foundLessons.push({
                            ...
                            period: card.period,
                            classroom: (classroom && classroom.short) || '',
